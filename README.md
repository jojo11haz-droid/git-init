...
- **Payments via Stripe Checkout (subscriptions).** Both patients and clinicians can subscribe. Patients: `STRIPE_PRICE_PATIENT_MONTHLY` / `STRIPE_PRICE_PATIENT_ANNUAL` (charged at signup). Clinicians: `STRIPE_PRICE_THERAPIST_MONTHLY` / `STRIPE_PRICE_THERAPIST_ANNUAL` (Stripe subscription with a **14-day free trial** — a card is put on file at signup but nothing is charged for 14 days, then it auto-charges unless cancelled). Set `STRIPE_SECRET_KEY` plus the relevant price ids (and, recommended, a webhook at `/api/stripe/webhook` with `STRIPE_WEBHOOK_SECRET`) to turn on real charging — until then, signup still works and creates the account for free. Card data never touches this server: it's entered on Stripe's hosted page. By design the app is **not** paywalled — accounts are created and usable, and crisis resources always show, regardless of payment state; subscription status is tracked (patients in their settings, clinicians under a "Billing" button). Use Stripe **test** keys until you're ready to go live, and confirm you're comfortable being the payment intermediary (tax/receipts/disputes) before switching to live keys.
